...
#    }
    ...
    mongodb_user { "User ${admin_username} on db admin":
      ensure        => present,
    ...
    Anchor['mongodb::server::end'] -> Mongodb_user["User ${admin_username} on db admin"]

    # Make sure it runs before other DB creation
    Mongodb_user["User ${admin_username} on db admin"] -> Mongodb::Db <| title != 'admin' |>
  }
  ...
  if $replset {
    ...
    if $replset_members and $replset_config {
      ...
      if $create_admin {
        Class['mongodb::replset'] -> Mongodb_user["User ${admin_username} on db admin"]
      }
